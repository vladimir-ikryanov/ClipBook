...
declare const deleteLinkImage: (imageFileName: string) => void;
...
export async function deleteItemImages(item: Clip) {
  ...
  if (item.type === ClipType.Link) {
    ...
    if (details?.imageFileName) {
      deleteLinkImage(details.imageFileName)
    }
    if (details?.faviconFileName) {
      deleteLinkImage(details.faviconFileName)
    }
    await deleteLinkPreviewDetails(url)
  }
}
...
export async function clear(keepFavorites: boolean): Promise<Clip[]> {
  ...
  // Delete the images and link preview images.
  for (const clip of history) {
    await deleteItemImages(clip)
  }
  history = []
...
export async function clearOlderThan(clipType: ClipType, days: number): Promise<Clip[]> {
  ...
  for (const clip of history) {
    ...
    // Skip items that are not of the specified type.
